...
income_class <- c("low_income", "mid_income", "high_income", "very_high_income")
...
# center city
ccd_sf <- read_sf("./inputs/CCD_Boundary.geojson") %>% st_transform(crs = 4269)
...
ggplot(income_tracts %>% filter(pct_change < 0.25), aes(x = income_bucket, y = pct_change)) +
  ...
  scale_y_continuous(labels = scales::percent) + 
  scale_x_discrete(labels = c("Low Income (<35k)", "Med Income (<50k)", "High Income (<100k)", "Very High (>100k)")) + 
